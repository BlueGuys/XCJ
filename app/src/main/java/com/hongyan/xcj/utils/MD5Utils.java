...
public class MD5Utils {
    ...
    public final static byte[] convertHexStringToByteArray(String source) {
        if (source != null) {
            int bytesLength = source.length() / 2;
            byte[] bytes = new byte[bytesLength];
            for (int bytesIndex = 0; bytesIndex < bytesLength; bytesIndex++) {
                bytes[bytesIndex] = (byte) (JavaTypesHelper.toRadixInt(
                        source.substring(bytesIndex * 2, bytesIndex * 2 + 2), 16));
            }
            return bytes;
        } else {
            return null;
